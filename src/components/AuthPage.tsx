...
export const APP_CONFIGS: Record<string, AppConfig & { githubOnly?: boolean }> = {
  astrova: {
    name: 'Astrova',
...
export default function AuthPage({ app = 'default' }: { app?: string }) {
  ...
  async function handleGitHub() {
    ...
    try {
      ...
      const sessionRes = await fetch('/api/auth/session', {
        ...
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ token, githubToken, githubUsername }),
      });
  ...
  const isGitHubOnly = config.githubOnly === true;
          ...
          {isGitHubOnly ? (
            <>
              ...
              <p className="mt-4 text-center text-xs text-zinc-500">
                GitHub access is required to analyze your repositories
              </p>
            ...
            <>
              ...
              <button
                ...
                disabled={loading}
                className={cn(
                  "w-full flex items-center justify-center gap-3 px-4 py-3 rounded-xl mt-3",
                  "bg-zinc-800 text-white font-medium text-sm border border-zinc-700",
